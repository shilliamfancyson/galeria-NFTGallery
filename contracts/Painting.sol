...
contract Painting is ERC721Enumerable, Ownable {
  ...
  uint256 public paintingSupply = 0;
  uint256 public robberSupply = 0;
  uint256 public paintingStolen = 0;
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    for (uint256 i = 1; i <= _mintAmount; i++) {
        ...
        if (generateSeed(paintingSupply+robberSupply,10) != 0){
          //mint a painting
          address recipient = selectReceipient();
          _safeMint(recipient, paintingSupply + 1);
          paintingSupply += 1;

        } else {
          //mint a robber
          address recipient = selectReceipient();
          _safeMint(recipient, robberSupply + 45001);
          robberSupply += 1;
        }
  ...
  function transferFrom(
        ...
        // Hardcode the Gallery's approval so that users don't have to waste gas approving
        if (_msgSender() != address(gallery)) {
            require(
  ...
  function setHotel(address _hotel) external onlyOwner {
        hotel = IHotel(_hotel);
    }
